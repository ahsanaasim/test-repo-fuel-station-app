...
## Objective
Build a full-featured preview of the 'Operator Flowboard' UI variation as described in the locked Design Brain prompt. Follow the specified vibrant, tablet-first, top-nav commerce shell with gallery-style dashboards, stepper wizards, and modal entry, using all required screens, layouts, and components.
...
Variation: Operator Flowboard
A vibrant, tablet-first, top-nav commerce shell with gallery-style dashboard cards, stepper wizards, and modal entry — never admin rails.
...
1. Landing ("/")
   - Layout: Top nav (Logo, ProductTitle), hero section with ProductOverviewText, large RoleCTAButtons (Operator Login, Manager Login, Accountant Login, Owner Login), FeatureHighlights as icon cards, footer with CopyrightNotice and SupportLink.
   - Status variants: loading (spinner in hero), error (error message + retry), plan_limit (plan message + contact support), permission_denied (message + GoToDashboard).
   - No credential forms here. Use vibrant hero, large CTAs, and merchandising-style feature highlights.

2. Login ("/auth/login")
   - Layout: Centered single-column form, large Fredoka heading, UsernameField, PasswordField, LoginButton (primary accent), links for ForgotPassword and Register below. Use Card with rounded-lg, shadow, and warm neutral background.
   - Status variants: loading (form disabled, spinner), error (error message above form), plan_limit (plan message + contact support), permission_denied (message + GoToDashboard).

3. Forgot Password ("/auth/forgot")
   - Layout: Centered card, Fredoka heading, InstructionText, EmailFormField, SubmitButton, BackToLoginLink. Confirmation screen: checkmark icon, "Check your email" title, ConfirmationText, BackToLoginLink.
   - Status variants: loading (form disabled, spinner), error (error message), plan_limit (plan message), permission_denied (message).

4. Register ("/auth/register")
   - Layout: Centered card, Fredoka heading, InstructionText, Name, Email, Password, Role (if allowed), Submit, BackToLoginLink. Use accent for primary actions.
   - Status variants: loading (form disabled, spinner), error (error message), plan_limit (plan message), permission_denied (message).

5. Operator Home ("/operator")
   - Layout: Top nav (Logo, minimal nav), main content is a gallery grid of dashboard cards:
     - Today's Sales (large, accent metric)
     - Fuel Left (by tank/type, with progress bars)
     - Active Shift (status badge)
     - Cash Variance (badge, green/red)
     - Low Tank Alerts (alert card, red/yellow)
   - Quick action buttons: "Add Sale" (opens modal), "End Shift" (stepper), "Correction Request" (modal).
   - Status variants: loading (skeleton cards, offline banner if offline), error (error card, retry), plan_limit (plan message), permission_denied (message).
   - Offline: show banner and unsynced data indicator.

6. Shift Start Wizard ("/operator/shift-start/step-1", "/step-2", "/confirm")
   - Layout: Stepper at top (StepIndicator), main card with list of pumps/tanks (each with Pump/Tank Number, Fuel Type, Opening Meter Reading field), Next/Back/Cancel buttons in sticky footer.
   - Step 2: Starting Cash Float entry.
   - Confirm: Summary card with all entered data, Confirm/Back.
   - Status variants: empty (no tanks, message), loading (spinner), error (error banner), plan_limit (plan message), permission_denied (message).

7. Fuel Sale Entry (Modal, "/operator/sale-entry")
   - Layout: Modal card, Fredoka heading, FuelType select, LitersSold, Amount, PaymentMethod, Timestamp (auto), Confirm/Cancel.
...
8. Correction Request (Modal, "/operator/correction-request")
   - Layout: Modal card, EntrySelectDropdown, ReasonTextArea, Submit, Close. Show status after submit (pending/approved/rejected badge).
   - Status variants: loading (modal disabled, spinner), error (error banner, retry/close), empty (no entries, message), plan_limit (plan message), permission_denied (message).

9. Shift End Wizard ("/operator/shift-end/step-1", "/step-2", "/step-3", "/confirmation")
   - Layout: Stepper at top, main card with list of pumps/tanks (Closing Meter Reading, Cash Amount, Card Amount), Continue/Back in sticky footer.
   - Step 2: Cash/Card totals.
   - Step 3: Summary card (sales, cash/card, variances, corrections), Submit/Back.
   - Confirmation: Checkmark icon, summary, Logout/Return to Dashboard.
   - Status variants: empty (no tanks, message), loading (spinner), error (error banner), plan_limit (plan message), permission_denied (message).

10. Manager Dashboard ("/manager")
    - Layout: Top nav (Logo, nav links: Dashboard, Shifts, Tanks, Deliveries, Expenses, Reports), main content is a gallery grid of dashboard cards (same as operator, but includes all tanks, all shifts, and quick links to workflows).
    - Status variants: loading (skeleton cards), error (error card, retry), plan_limit (plan message), permission_denied (message).

11. Shift Verification ("/manager/shifts")
    - Layout: Top nav, page header (Fredoka, "Shift Verification"), filter bar (DateRange, Operator, Status, Station), main region is a Table (shadcn Table) with columns: Shift Date/Time, Operator Name, Status (badge), Opening/Closing Meter, Total Liters Sold, Total Sales, Cash/Card, Variance, Correction Requested (badge), row actions (Verify, Reject, View Details). Pagination below table.
    - Status variants: empty (friendly message), loading (skeleton rows), error (error banner, retry), plan_limit (plan message), permission_denied (message).

12. Shift Verification Detail ("/manager/shifts/:shiftId")
    - Layout: Drawer or modal with full shift details, approve/reject/correction actions, summary cards for sales, meters, cash/card, variance, correction status.

13. Tank Inventory ("/manager/tanks")
    - Layout: Top nav, page header, gallery grid of tank cards (Tank Number, Fuel Type, Capacity, Current Level, Last Updated, Low Alert badge if needed). Click tank for detail drawer.
    - Status variants: empty (prompt to add tank), loading (skeleton cards), error (error banner), plan_limit (plan message), permission_denied (message).

14. Fuel Delivery ("/manager/deliveries")
    - Layout: Top nav, page header, Table (Supplier, Date/Time, Fuel Type, Liters, Tank, Invoice Number), Add Delivery button (opens modal).
    - Status variants: empty (prompt to add delivery), loading (skeleton rows), error (error banner), plan_limit (plan message), permission_denied (message).

15. Cash Reconciliation ("/manager/reconciliation")
    - Layout: Top nav, page header, Table (Shift, Expected Cash, Counted Cash, Card Amount, Variance), Add Reconciliation button.
    - Status variants: empty (prompt to add), loading (skeleton rows), error (error banner), plan_limit (plan message), permission_denied (message).

16. Expenses ("/manager/expenses")
    - Layout: Top nav, page header, Table (Date, Category, Amount, Note, Entered By), Add Expense button.
    - Status variants: empty (prompt to add), loading (skeleton rows), error (error banner), plan_limit (plan message), permission_denied (message).

17. Reports ("/manager/reports")
    - Layout: Top nav, page header, report cards (Sales, Inventory, Reconciliation, Expenses, Variances), export buttons (CSV/PDF).
    - Status variants: loading (spinner), error (error banner), plan_limit (plan message), permission_denied (message).

18. Accountant Workspace ("/accountant")
    - Layout: Top nav, page header, cards for Expenses, Cash Reconciliation, Reports. No access to shift or sales workflows.

19. Owner Overview ("/owner")
    - Layout: Top nav, page header, full dashboard cards for all areas, quick links to settings, audit log, and all reports.

20. Audit Log ("/owner/audit-log")
    - Layout: Top nav, page header, Table (Timestamp, User, Action, Details).

21. Settings (Drawer, "owner/settings")
    - Layout: Drawer with role management, station config.
...
Index URL: https://github.com/ahsanaasim/test-repo-fuel-station-app/blob/ai-build/preview-var1-issue-1-implement-operator-flowboard-ui-preview-build-fu/.ops4/design-system/DESIGN_SYSTEM.md
...
## Requirements
- Implement every screen and route listed in the locked Design Brain prompt, including all dashboard, entry, modal, and management pages.
- Use the specified vibrant merchandising/gallery storefront aesthetic, color palette, and typography (Fredoka/Nunito).
- Apply the top navigation shell (never sidebar), with responsive layouts for Android tablet and desktop.
- Use Shadcn/ui and Tailwind CSS for all components, matching the design system and layout archetypes.
- Implement all status variants (loading, error, empty, plan limit, permission denied) for each screen.
- Populate each page with realistic sample data matching the provided page-by-page data points and field counts.
- Include all modal flows, stepper wizards, and dashboard card galleries as described.
...
## Acceptance Criteria
- All screens and flows from the locked prompt are present and visually match the described layouts and components.
- All status variants are implemented and testable for each page.
- Sample data is present and matches the required fields per page.
- UI is responsive and touch-friendly on Android tablet and desktop.
- No sidebar navigation or admin rails are present; top navigation is used throughout.
- Lint passes
- Build passes
- A PR is opened from the preview branch to the default branch (not merged)
...
`ai-build/preview-var1-issue-1-implement-operator-flowboard-ui-preview-build-fu`
...
PR title should be:
AI Build: Implement Operator Flowboard UI Preview Build (Full App)
