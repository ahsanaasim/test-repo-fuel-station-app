...
export default function ManagerReconciliationPage() {
  ...
  const totalVariance = reconList.reduce((sum, recon) => sum + recon.variance, 0);
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <div className="mb-6">
          <EditorialListHeader
            ...
            description="Match counted cash and card totals against expected shift sales."
            badge={
              <Badge
                variant={totalVariance < 0 ? "danger" : totalVariance > 0 ? "warning" : "success"}
              >
                {formatCurrency(totalVariance)} variance
              </Badge>
            }
            actions={
        ...
        {(showReady || showOffline) && (
          <div className="space-y-4">
            <EditorialSection
              ...
              summary={`${reconList.length} reconciliations recorded`}
              tone={totalVariance < 0 ? "danger" : totalVariance > 0 ? "warm" : "success"}
              cta={
                <DialogTrigger asChild>
                  <Button size="lg">
                    <Plus className="h-4 w-4" />
            ...
            >
              <p className="text-sm opacity-80">
                Count the cash drawer, confirm card totals, and record the variance before the
                shift is closed for reporting.
              </p>
            </EditorialSection>
