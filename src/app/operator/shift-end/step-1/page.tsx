...
export default function ShiftEndStep1Page() {
  ...
  return (
    <AppShell navItems={operatorNav} homeHref="/operator">
      <Stepper steps={SHIFT_END_STEPS} current={1} />
      <PageHeader
        title="Closing meter readings"
        description="Enter the closing totalizer, cash, and card amounts for each pump."
      />
        ...
        <TwoColumn
          main={
            <div className="space-y-4">
              {showOffline && (
                <Badge variant="warning">Offline · entries queue until reconnect</Badge>
              )}
              ...
              <div className="grid gap-4">
                {pumps.map((pump) => {
                  ...
                  return (
                    <Card key={pump.id}>
                      <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] bg-[var(--muted)]/30 pb-4">
                        <div className="flex items-center gap-3">
                          <span className="flex h-11 w-11 items-center justify-center rounded-lg bg-[var(--accent)]/10 text-[var(--accent)]">
                      ...
                      <CardContent className="grid gap-4 pt-5 sm:grid-cols-3">
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              updateEntry(pump.id, "closingMeter", e.target.value)
                            }
                          />
                        </div>
                        <div className="space-y-2">
