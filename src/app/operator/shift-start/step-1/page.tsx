...
export default function ShiftStartStep1Page() {
  ...
  return (
    <AppShell navItems={operatorNav} homeHref="/operator">
      <Stepper steps={SHIFT_START_STEPS} current={1} />
      <PageHeader
        title="Opening meter readings"
        description="Record the meter on each pump before sales begin. Values should match the physical display."
      />
          ...
          main={
            <div className="space-y-4">
              ...
              <div className="grid gap-4">
                {pumps.map((pump) => (
                  <Card key={pump.id} className="overflow-hidden">
                    <CardHeader className="flex flex-row items-center justify-between gap-3 border-b border-[var(--border)] bg-[var(--muted)]/30 pb-4">
              ...
              <WizardFooter
                backHref="/operator"
                backLabel="Cancel"
                nextHref="/operator/shift-start/step-2"
                nextLabel="Next: Cash float"
              />
            </div>
          }
          rail={
