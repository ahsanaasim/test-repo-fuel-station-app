    ...
    <div className="sticky bottom-0 z-10 -mx-4 mt-6 border-t border-[var(--border)] bg-white/95 px-4 py-4 backdrop-blur-md sm:-mx-6 sm:px-6">
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
        ...
        <Button asChild size="lg" className="w-full sm:w-auto">
          <Link href={nextHref}>Next: Review</Link>
        </Button>
      </div>
    </div>
  );
...
export default function ShiftStartStep2Page() {
  ...
  return (
    <AppShell navItems={operatorNav} homeHref="/operator">
      <Stepper steps={SHIFT_START_STEPS} current={2} />
      <PageHeader
        title="Starting cash float"
        description="Count the cash in the drawer before the first sale. This becomes your opening balance."
      />
        ...
        <TwoColumn
          main={
            <div className="space-y-4">
              {showOffline && (
                <Badge variant="warning">Offline · float saves on this device</Badge>
              )}
              ...
              <WizardFooter
                backHref="/operator/shift-start/step-1"
                nextHref="/operator/shift-start/confirm"
              />
            </div>
          }
          rail={
