...
export function StatusBanner() {
  ...
  if (isOffline) {
    return (
      <div className="mb-4 flex items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-amber-900">
        <WifiOff className="h-5 w-5 shrink-0" />
        <div className="flex-1 text-sm">
          <p className="font-semibold">You&apos;re offline</p>
          <p>Sales stay on this device and sync when you reconnect. 2 entries waiting.</p>
        </div>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="mb-4 flex flex-wrap items-center gap-3 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-rose-900">
        <AlertTriangle className="h-5 w-5 shrink-0" />
        <div className="flex-1 text-sm">
          <p className="font-semibold">Something went wrong</p>
          <p>We couldn&apos;t load the latest station data. Try again in a moment.</p>
        </div>
